...
# TEST_RESULTS_DIR="/home/jxu680/image-concept-compression/data_runs/feb/k4_LVIS_250_maskclip"
TEST_RESULTS_DIR="/home/jxu680/image-concept-compression/k4_tests_LVIS_MASKCLIP_pk(10)_n(1000)_idx(flat)"
...
# Function to wait for all background processes and clear PIDs
wait_and_clear() {
    wait
    pids=()
}
...
for test_file in "$TEST_RESULTS_DIR"/*.pkl; do
    if [ -f "$test_file" ]; then
        echo "Processing first set of tests for: $test_file"
...
# Wait for the first set to complete
wait_and_clear

# Run the second set of tests
# for test_file in "$TEST_RESULTS_DIR"/*.pkl; do
#     if [ -f "$test_file" ]; then
#         echo "Processing second set of tests for: $test_file"
#         run_test_five_128 "$test_file"
#         run_test_one "$test_file"
#     fi
# done

# # Wait for the second set to complete
# wait_and_clear
