...
def perform_search(features, p_k, kmeans, pqq, packd, img_concept_bitmap, all_images, 
    ...
    for key, concept_feature in features.items():
        ...
        selected_clusters.add(cluster_ids[0])

    selected_clusters = list(selected_clusters)
    valid_imgs = np.all(img_concept_bitmap[:, selected_clusters], axis=0)
    ...
    # Each query will have a set of cluster ids that are closest to it
    for query_i, (distances, cluster_ids) in enumerate(closest_centroids):
        # assert len(cluster_ids) == n_probes
        logger.info(f"(US) Query {query_i} w/: {cluster_ids.shape}")

    for key, concept_ids in key_to_concept_ids.items():
        logger.info(f"(US) Key {key} maps to concepts {len(concept_ids)}")
    ...
    # print('US: Number of matching images', len(matching_images))
    logger.info(f"(US) Number of matching images: {len(matching_images)}")
    ...
    logger.info(f"(US) Time to compute distance table: {time.time() - t1}")
    ...
    logger.info(f"(US) Time to compute distances: {time.time() - t1}")
    ...
    img_distances = {}
    for img_idx in matching_images:
        img_distances[img_idx] = 0
        for key in all_keys:
            img_distances[img_idx] += all_distances[(img_idx, key)]

    logger.info("(US) BEEBOP")
    logger.info(f"(US) Time to score images: {time.time() - t1}")
    # logger.info(f"(US) all_distances sorted: {img_distances}")

    sorted_images = sorted(img_distances, key=img_distances.get)[:p_k]
    # Pair up the images with their distances in a logger info
    logger.info(f"(US) Sorted images: {[(img, img_distances[img]) for img in sorted_images[:p_k]]}")
    sorted_images = [all_images[idx] for idx in sorted_images]
    logger.info(f"(US) Sorted images: {sorted_images[:p_k]}")
    return sorted_images, img_distances, counter

    # Convert heap to sorted list of top p_k images
    # image_distances_named = {img_idx: -dist for dist, img_idx in img_distances}
    # print('US: Distances', image_distances_named)
    # sorted_images = [all_images[idx] for _, idx in nlargest(p_k, img_distances)]
    # print('US: Sorted images', sorted_images[:p_k])
    # logger.info(f"(US) Sorted images: {sorted_images[:p_k]}")
    # return sorted_images, image_distances_named, debug_reconstruction
    # return sorted_images, image_distances_named, counter
