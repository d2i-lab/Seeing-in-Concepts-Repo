...
def flat_multisearch(search_index, features, p_k, 
    ...
    if hasattr(search_index, 'invlists'):
        ...
        max_search_radius =  max([invlists.list_size(l) for l in range(invlists.nlist)])
        # search_radius = max_search_radius * search_index.nprobe
        search_radius = p_k
        log_prefix = '(PQ)'

        logger.info(log_prefix, 'Index settings')
        logger.info(log_prefix, 'nlist', invlists.nlist)
        logger.info(log_prefix, 'max_search_radius', max_search_radius)
        logger.info(log_prefix, 'nprobe', search_index.nprobe)
        logger.info(log_prefix, 'Features', len(features))

    if is_gt:
        search_radius = search_index.ntotal
        max_search_radius = search_index.ntotal
    ...
    while True:
        ...
        if isinstance(search_index, faiss.IndexIVFPQ):
        ...
        logger.info(log_prefix, indices.shape, "is the shape of indices")
        logger.info(log_prefix, len(features), "is the number of features")
        logger.info(log_prefix, search_radius, "is the search radius")

        for i, (key, feature) in enumerate(features.items()):
            feature_indices = indices[i] # For this batch of features (radius)
            feature_distances = distances[i] # For this batch of features
            img_names = [vec_to_img[idx] for idx in feature_indices] # Image names for batch
            potential_candidates.append(set(img_names))

            # Store distances for each image
            # img_names may repeat.
            # but we only care about the closest one, which is the first one.
            seen_imgs = set()
            for j, img in enumerate(img_names):
                if img in seen_imgs:
                    continue
                seen_imgs.add(img)
                temp_distances[img][key].append((feature_distances[j], feature_indices[j]))

            logger.info(log_prefix, f'{i}| {key} -> {len(temp_distances[img][key])}')

        logger.info(log_prefix, 'Potential candidates', len(potential_candidates))
    ...
    if len(candidates) < p_k:
        logger.info(log_prefix, f"Only found {len(candidates)} candidates, returning all")

    logger.info(log_prefix, 'Number of candidates', len(candidates))

    # After set intersection, every candidate must have all features. Do assertion here.
    # for img in candidates:
        # assert all(key in all_distances[img] for key in features), f"Missing key in {img}"
        # No extra keys
        # assert all(key in features for key in all_distances[img]), f"Extra key in {img}"
    ...
    for img in all_distances:
        ...
        if exclusive_matching:
            ...
            img_scores[img] = 0
            distances_idx = all_distances[img]
            # logger.info(log_prefix, 'Distances_idx', distances_idx)
            # for key in distances_idx:
            for key in features:
                img_scores[img] += distances_idx[key][0][0]

            # print('Distance_idxs', distances_idx[key])
            # logger.info(log_prefix, 'Scores', distances_idx[key])
    ...
    logger.info(log_prefix, 'Sorted image scores', sorted_images[:p_k])
    sorted_images = [img for img, _ in sorted_images[:p_k]]
    logger.info(log_prefix, 'Sorted images', sorted_images[:p_k])
    return sorted_images[:p_k], sorted_images, counter
